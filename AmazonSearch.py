...
class SearchTest(unittest.TestCase):
    ...
    def test_search_text_field_tab_index(self):
        search_field = self.driver.find_element_by_id("twotabsearchtextbox")
        # check maxlength attribute is set to 128
        self.assertEqual( "6", search_field.get_attribute( "tabindex" ) )

    def test_search_magnifying_symbol(self):
        self.search_field = self.driver.find_element_by_class_name("nav-input")
        self.assertEqual("field-keywords", self.search_field.get_attribute("name"))
